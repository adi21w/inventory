...
7. Jalankan perintah `flutter doctor` untuk memeriksa kesiapan device
8. Arahkan Flutter SDK, jika menggunakan Windows jalankan perintah `flutter config --android-sdk "C:\Users\[YOUR PC NAME]\AppData\Local\Android\Sdk"`
9. Restart VSCode atau tutup lalu buka kembali VSCode
10. Masuk ke folder `lib` > `config`, dan buka file `constants.dart`
11. Ubah bagian di **line 4** `static const String baseUrl = '[YOUR HOST]';`
12. Pada menu VSCode pilih `Run` > `Start Debuggin` atau bisa tekan tombol `F5`
13. Jika Debug tidak berjalan, gunakan perintah `flutter run -d chrome`
14. Login menggunakan username `X0001` dan password `147852` untuk mencoba
