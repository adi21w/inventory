...
$this->registerJsFile("@web/js/apps/adjustment/form-minus.js?v" . date('Ymd'), ['depends' => [\yii\web\YiiAsset::class]]);
?>
<div class="component-page-form" data-route-parent="/adjustment-minus">
            ...
            <div class="row">
                <div class="col-md-3">
                    <?= $form->field($model, 'iGudangId')->widget(Select2::classname(), [
                        'data' => ArrayHelper::map(Warehouses::find()->where($model->isNewRecord ? ['eDeleted' => 'Tidak'] : ['iId' => $model->iGudangId])->all(), 'iId', 'vNama'),
                ...
                <div class="col-md-3">
                    <?= $form->field($model, 'eKategori')->dropDownList(
                        ['STOCK OPNAME' => 'STOCK OPNAME', 'HILANG' => 'HILANG', 'PEMBETULAN BATCH' => 'PEMBETULAN BATCH', 'EXPIRED' => 'EXPIRED', 'PEMUSNAHAN' => 'PEMUSNAHAN']
                    );
                    ?>
                </div>
            </div>
    ...
    <div class="card mb-4">
                        ...
                        <div class="item panel panel-default">
                            <div class="panel-body" style="padding:10px;">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <?= $form->field($modelDetail, "[{$index}]vBatch")->dropDownList(
                                            ($model->isNewRecord) ? [] : [$modelDetail->vBatch => $modelDetail->vBatch],
                                            ['required' => true, 'data-input' => 'batch']
                                        );
                                        ?>
                                    </div>
                                    <div class="col-md-2">
                                        <?= $form->field($modelDetail, "[{$index}]dExpired")->textInput(['required' => true, 'readonly' => true, 'data-input' => 'expired']) ?>
                                    </div>
                                    <div class="col-md-1">
                                        <?= $form->field($modelDetail, "[{$index}]iQty")->textInput(['required' => true, 'data-format' => 'number', 'maxlength' => 4, 'data-input' => 'qty', 'data-max' => 0]) ?>
                                    </div>
                                    <div class="col-sm-1">
        ...
        <div class="block-layer" id="block-layer">
            <h3 class="text-center">Pilih Gudang terlebih dahulu</h3>
        </div>
