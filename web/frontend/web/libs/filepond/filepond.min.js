...
var files=Array.from(root.element.files).map(function(file){file._relativePath=file.webkitRelativePath;return file});setTimeout(function(){props.onload(files);resetFileInput(root.element)},250)};root.element.addEventListener('change',root.ref.handleChange)};var setAcceptedFileTypes=function setAcceptedFileTypes(_ref2){var root=_ref2.root,action=_ref2.action;if(!root.query('GET_ALLOW_SYNC_ACCEPT_ATTRIBUTE'))return;attrToggle(root.element,'accept',!!action.value,action.value?action.value.join(','):'')};var toggleAllowMultiple=function toggleAllowMultiple(_ref3){var root=_ref3.root,action=_ref3.action;attrToggle(root.element,'multiple',action.value)};var toggleDirectoryFilter=function toggleDirectoryFilter(_ref4){var root=_ref4.root,action=_ref4.action;attrToggle(root.element,'webkitdirectory',action.value)};var toggleDisabled=function toggleDisabled(_ref5){var root=_ref5.root;var isDisabled=root.query('GET_DISABLED');var doesAllowBrowse=root.query('GET_ALLOW_BROWSE');var disableField=isDisabled||!doesAllowBrowse;attrToggle(root.element,'disabled',disableField)};var toggleRequired=function toggleRequired(_ref6){var root=_ref6.root,action=_ref6.action;if(!action.value){attrToggle(root.element,'required',!1)}else if(root.query('GET_TOTAL_ITEMS')===0){attrToggle(root.element,'required',!0)}};var setCaptureMethod=function setCaptureMethod(_ref7){var root=_ref7.root,action=_ref7.action;attrToggle(root.element,'capture',!!action.value,action.value===!0?'':action.value)};var updateRequiredStatus=function updateRequiredStatus(_ref8){var root=_ref8.root;var element=root.element;if(root.query('GET_TOTAL_ITEMS')>0){attrToggle(element,'required',!1);attrToggle(element,'name',!1);var activeItems=root.query('GET_ACTIVE_ITEMS');var hasInvalidField=!1;for(var i=0;i<activeItems.length;i++){if(activeItems[i].status===ItemStatus.LOAD_ERROR){hasInvalidField=!0}}
root.element.setCustomValidity(hasInvalidField?root.query('GET_LABEL_INVALID_FIELD'):'')}else{attrToggle(element,'name',!0,root.query('GET_NAME'));var shouldCheckValidity=root.query('GET_CHECK_VALIDITY');if(shouldCheckValidity){element.setCustomValidity('')}
