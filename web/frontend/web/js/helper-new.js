(function(window) {
    ...
    Helper_Page.prototype.createHTML = {
        ...
        table: function(data, opts = {}) {
            const creator = this
            let headers;
            if ((opts.hasOwnProperty('topHead')) && (opts.topHead)) {
                headers = Object.keys(data[0]).filter(key => data[0][key] !== null);
            }

            const headerRow = creator.tag('tr', headers.map(h => `<th>${h.toUpperCase()}</th>`).join(''))
            const headerTable = creator.tag('thead', headerRow)

            const bodyRow = data.map(item => creator.tag('tr', headers.map(key => `<td>${item[key]}</td>`).join(''))).join('')
            const bodyTable = creator.tag('tbody', bodyRow)

            const html = creator.tag('table', headerTable+bodyTable, {class: "table table-bordered"})
            return html;
        }
    }
    ...
    Helper_Page.prototype.renderView = function(data) {
        ...
        $.each(data, function(k, v) {
            ...
            } else if (('format' in v) && (v.format == 'table')) {
                subListItem += creator.tag('div', creator.tag('label', v.label, {'class': 'col-form-label'}), {class: 'col-md-2'})
                subListItem += creator.tag('div', creator.table(v.value, {topHead: true}), {class: 'col-md-10'})
            } else {
    ...
    Helper_Page.prototype.dynamicGetID = function(data) {
